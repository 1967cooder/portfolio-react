...
const Home = () => {
  ...
  // Избираме по един проект от всеки тип
  const cvProject = data.find(
    (project) => project.title === "CV" && project.featured
  );
  ...
  // Третата карта – Education
  const educationProject = data.some(
    (project) => project.type === "education" && project.featured
  )
    ? {
        id: "education-home",
  ...
  // Създаваме масива с проп `hideDescription` за всяка карта
  const featuredProjectsHome = [
    { ...cvProject, hideDescription: true },
    { ...proudProject, hideDescription: true },
    { ...educationProject, hideDescription: true }, // description скрито за Education
  ].filter(Boolean);
      ...
      >
        {featuredProjectsHome.map((project) => (
          <ProjectCard
            ...
            hideDescription={project.hideDescription}
          />
        ))}
      </Box>
